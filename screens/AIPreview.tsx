...
import { ArrowLeft, Sparkles, Image as ImageIcon, RotateCcw, MessageSquare, Check, Palette, Download, Eye, Clock, History, Layout, Lightbulb, Plus, Grid, Wand2, X, Info, ChevronDown, ChevronUp, Hash, Hammer, Link as LinkIcon, Layers, Upload } from 'lucide-react';
...
export const AIPreview: React.FC<AIPreviewProps> = ({ navigate, goBack }) => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
          ...
          reader.onloadend = () => {
              // Set the data URL as the refImageUrl
  ...
  const urlToBase64 = async (url: string): Promise<string> => {
    // If it's already a data URL, return the base64 part directly
  ...
  const handleGenerate = async () => {
    ...
    if (!activePrompt.trim() && !refImageUrl && !isRefining) return;
    ...
    try {
      ...
      if (isRefining && generatedImage) {
        // Img2Img (Refinement using generated image)
        const base64Data = generatedImage.split(',')[1];
        contents = [{
            parts: [
                { text: `Modify the interior design based on the provided image. ${refinePrompt}. 
        ...
        // Normal generation (Text-to-Image OR ReferenceImage-to-Image)
        ...
        if (refBase64 && !isRefining) {
             // Reference Image + Text (Img2Img)
             contents = [{
                 parts: [
                    { text: `Redesign the interior space shown in the image. 
                             Maintain the structural layout but change the design.
                             Style: ${selectedStyle} (${styleDesc}). 
                             Color: ${selectedColor} (${colorDesc}).
                             Details: ${prompt}. ${qualityPromptSuffix} ${safetySuffix}` },
  ...
  const isDataUrl = refImageUrl.startsWith('data:');
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2 pt-1">
                ...
                <div className="flex gap-2">
                    <input 
                        type="text"
                        placeholder={isDataUrl ? "이미지 파일이 선택되었습니다" : "이미지 URL 입력"}
                        className={`flex-1 p-3 bg-white border border-gray-200 rounded-xl text-sm focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none transition-all placeholder:text-gray-400 ${isDataUrl ? 'text-gray-400 bg-gray-50' : ''}`}
                        value={isDataUrl ? '' : refImageUrl}
                        onChange={(e) => setRefImageUrl(e.target.value)}
                        disabled={loading || isDataUrl}
                    />
                    ...
                    <button 
                        ...
                        disabled={loading}
                        className="px-3 bg-gray-100 rounded-xl text-gray-600 hover:bg-gray-200 active:bg-gray-300 transition-colors flex items-center justify-center border border-gray-200"
                        title="내 이미지 업로드"
                    >
                ...
                {refImageUrl && (
                    <div className="relative w-full h-32 bg-gray-100 rounded-lg overflow-hidden border border-gray-200 mt-2 group">
                        <img src={refImageUrl} alt="Reference" className="w-full h-full object-cover opacity-90" />
                         <div className="absolute inset-0 flex items-center justify-center bg-black/10">
                            <span className="text-[10px] font-bold text-white bg-black/50 px-2 py-1 rounded backdrop-blur-sm">
                                {isDataUrl ? '내 이미지 적용됨' : '참고 이미지 링크 적용됨'}
                            </span>
                         </div>
                        <button 
                            onClick={() => {
                            ...
                            className="absolute top-2 right-2 bg-black/50 text-white rounded-full p-1.5 hover:bg-red-500 transition-colors shadow-sm"
                        >
                            <X className="w-3 h-3" />
                        </button>
                    </div>
